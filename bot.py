...
BingX Signal Bot v5
===================
FIX PRINCIPAL: señales ahora se generan con filtros graduales.
Telegram: alertas ricas igual que el bot de referencia (CVD, BB, MTF, vol).
"""
...
_STATE: dict = {"html": (
    ...
    "<h2>⚡ BingX Bot v5</h2><p>Iniciando...</p></body></html>"
)}
...
# ── Telegram: formato rico como el bot de referencia ─────────── #
...
async def _send_signal_alert(client, sig: dict):
    """Envía alerta Telegram con el formato del bot de referencia."""
    s     = sig
    emoji = _score_emoji(s["score"])
    lvl   = _level_label(s["score"])
    dir_e = "🟢" if s["direction"] == "LONG" else "🔴"

    # Línea de métricas compacta
    z_score = round((s["cvd_pct"] - 50) / 10, 2)  # CVD normalizado
    metrics = (
        f"BB:{s['bb_width']:.1f}% "
        f"Z:{z_score:+.2f} "
        f"CVD:{s['cvd_pct']:.0f}% "
    ...
    sigs_lines = "\n".join(
        f"  {'🎯' if 'BB' in l else '🔥' if 'Vol' in l else '🌊' if 'CVD' in l else '✅' if 'MTF' in l else '🚀'} {l}"
        for l in s["active_sigs"]
    )
    if not sigs_lines:
        sigs_lines = f"  📊 {s['signal_type']}"

    msg = (
        f"{emoji} <b>{lvl} {s['symbol']} — {s['score']:.0f}%</b>\n"
        f"$ ${s['price']:.6g} | 24h: {s['mean_pnl']*100:+.2f}%\n"
        f"{metrics}\n"
        f"Señales:\n{sigs_lines}\n"
        f"{'  🔥 Vol acelerado' if s.get('bb_expansion',0)>10 else ''}"
...
def _render(s: dict) -> str:
    ...
    def stat(lbl, val, col="#00ff88", sub=""):
        return (f'<div style="background:#0d1117;border:1px solid #1a2535;border-radius:6px;'
                ...
                f'<div style="font-size:10px;letter-spacing:2px;color:#4a5568;'
    ...
    wr = perf.get("win_rate",0); pf = perf.get("profit_factor",0)
    stats = "".join([
        ...
        stat("PBR Acum.",     f"{total_pbr:+.5f}", "#00ff88" if total_pbr>=0 else "#ff3355"),
    ])
    ...
    for sg in s.get("signals",[]):
        ...
        srows += (
            ...
            f'<td style="{TD};color:#4a5568">{mtf}</td>'
    ...
    sigs_html = (
        ...
        '⚠️ Sin señales — scanner activo, ajusta MIN_SIGNAL_SCORE si necesitas más</div>'
    )
    ...
    for sym,snap in s.get("ob",{}).items():
        ...
        imb=snap.get("imbalance_ratio",1.); bias=snap.get("bias","?")
        bc="#00ff88" if bias=="BULLISH" else "#ff3355" if bias=="BEARISH" else "#ffcc00"
        bd=snap.get("bid_delta_pct",0); bdc="#00ff88" if bd>0 else "#ff3355"
        bw=len(snap.get("bid_walls",[])); aw=len(snap.get("ask_walls",[]))
...
<title>BingX Signal Bot v5</title>
...
<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 22px;
  ...
  <div>
    <div style="font-size:18px;font-weight:700;letter-spacing:3px;color:#00ff88">⚡ BINGX SIGNAL BOT v5</div>
    <div style="font-size:10px;color:#4a5568;letter-spacing:2px;margin-top:1px">CVD · BB SQUEEZE · MTF · ORDER BOOK · RISK MANAGER</div>
  </div>
  <div style="display:flex;gap:12px;align-items:center">
    <span style="font-size:10px;color:#4a5568">Ciclo #{s.get("cycle",0)} · {s.get("updated","—")}</span>
    {m}
  </div>
</div>
...
class TradingBot:
    ...
    async def run(self):
        ...
        log.info(f"Bot v5 iniciado — {mode}  (health en :{_PORT})")
        try:
            await self.client.initialize()
        except Exception as e:
            log.error(f"initialize(): {e}")

        await self.client.notify(f"⚡ <b>Bot v5 iniciado</b> — {mode}\nScan: {SCAN_INTERVAL}s")

        self.running = True
        loop = asyncio.get_event_loop()
        for sig in (signal.SIGTERM, signal.SIGINT):
            try: loop.add_signal_handler(sig, lambda: setattr(self,"running",False))
    ...
    async def _tick(self):
        ...
        for sym in hot:
            try:
                ob_snap = await self.ob.analyze(sym)
                sig = await self.signal_engine.evaluate(self.client, sym, ob_snap)
                if sig:
                    signals.append(sig)
                    # Enviar alerta Telegram inmediatamente
        ...
        _STATE["html"] = _render({
            "symbols":hot, "signals":signals, "trades":trades,
            "cycle":self._cycle,
            "updated":datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC"),
            "daily":daily, "perf":perf, "ob":ob_dict,
        })
