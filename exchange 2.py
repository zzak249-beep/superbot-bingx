...
def calcular_cantidad(par: str, balance: float, precio: float) -> float:
    ...
    # Margen fijo de 8 USDT por trade
    # notional = margen * leverage → cantidad = notional / precio
    import math
    MARGEN_FIJO = 8.0
    leverage    = getattr(config, "LEVERAGE", 2)

    if balance < MARGEN_FIJO:
        print(f"[EXCHANGE] Balance insuficiente: ${balance:.2f} < ${MARGEN_FIJO} margen requerido")
        return 0.0

    notional_objetivo = MARGEN_FIJO * leverage
    ...
    print(f"[EXCHANGE] Margen: ${margen_real:.2f} USDT | Notional: ${cantidad * precio:.2f} | Lev:{leverage}x")
    return cantidad
