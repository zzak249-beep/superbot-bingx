...
FIX FIRMA: BingX requiere timestamp AL FINAL del query string (no sorted)
"""
...
def _build_qs(params: dict) -> str:
    """
    BingX REQUIERE timestamp al FINAL del query string.
    sorted() rompe la firma — NO usar sorted().
    """
    ts = params.pop("timestamp", None)
    parts = [f"{k}={v}" for k, v in params.items()]
    if ts is not None:
        parts.append(f"timestamp={ts}")
        params["timestamp"] = ts  # restaurar
    return "&".join(parts)
...
def get_balance() -> float:
    ...
    print(f"[BALANCE] raw: code={resp.get('code')} | {str(resp.get('data',''))[:200]}")

    val = _buscar_float(resp, ["availableMargin", "available", "free"])
    if val > 0:
        print(f"[BALANCE] ✓ disponible = ${val:.2f}")
        return val
    ...
    print(f"[BALANCE] ✗ balance=0. Respuesta completa: {resp}")
    return 0.0
