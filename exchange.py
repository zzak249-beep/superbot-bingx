...
def set_leverage(par: str, leverage: int) -> bool:
    ...
    # Paso 1: margen aislado
    r_margin = _post("/openApi/swap/v2/trade/marginType", {
        "symbol":     par,
        "marginType": "ISOLATED",
    })
    code_m = r_margin.get("code")
    if code_m not in (0, 200038):  # 200038 = ya está en ese modo
        print(f"[EXCHANGE] marginType {par}: code={code_m} {str(r_margin.get('msg',''))[:80]}")

    # Paso 2: apalancamiento (hedge mode: LONG y SHORT por separado)
...
# ─────────────────────────────────────────────────────
# ÓRDENES — MODO HEDGE + MARGEN AISLADO
# LONG:  side=BUY,  positionSide=LONG
# SHORT: side=SELL, positionSide=SHORT
# SL/TP: positionSide + quantity (hedge requiere qty, no closePosition)
# ─────────────────────────────────────────────────────
...
def abrir_short(par: str, cantidad: float, precio_entrada: float,
                sl: float, tp: float) -> dict:
    """Abre posición SHORT — hedge + aislado"""
    if getattr(config, "MODO_DEMO", False):
