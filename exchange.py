...
FIXES:
  - _get/_post usan sorted() para firma consistente → arregla balance=0
  - parsear_klines soporta dicts (BingX v3) Y arrays (BingX legacy)
  - _extraer_balance_usdt maneja todos los formatos de respuesta BingX
"""
...
def _get(path: str, params: dict = None, auth: bool = True) -> dict:
    ...
    if auth:
        ...
        # CRÍTICO: sorted() garantiza firma correcta en todos los endpoints
...
def _post(path: str, params: dict) -> dict:
    params["timestamp"] = int(time.time() * 1000)
    # CRÍTICO: sorted() garantiza firma correcta
    query_string = "&".join(f"{k}={v}" for k, v in sorted(params.items()))
    signature    = _sign(query_string)
    url = f"{BASE_URL}{path}?{query_string}&signature={signature}"
    try:
...
def get_balance() -> float:
    ...
    if config.MODO_DEBUG:
        print(f"[EXCHANGE] balance raw: {str(resp)[:300]}")
    ...
    # Tercer intento: endpoint v3
    try:
        resp3 = _get("/openApi/swap/v3/user/balance", {"currency": "USDT"}, auth=True)
        val3 = _extraer_balance_usdt(resp3, "availableMargin")
        if val3 > 0:
            return val3
    except:
        pass

    print(f"[EXCHANGE] ⚠ balance=0 — respuesta: {str(resp)[:300]}")
    return 0.0
