...
_position_mode_cache: dict = {}  # {par: "HEDGE" | "ONEWAY"}


def _get_position_mode() -> str:
    """Detecta si la cuenta está en modo ONE-WAY o HEDGE"""
    try:
        resp = _get("/openApi/swap/v2/trade/positionSide/dual", {})
        dual = resp.get("data", {})
        if isinstance(dual, dict):
            is_hedge = dual.get("dualSidePosition", False)
        else:
            is_hedge = False
        mode = "HEDGE" if is_hedge else "ONEWAY"
        print(f"[EXCHANGE] Modo posición detectado: {mode}")
        return mode
    except Exception as e:
        print(f"[EXCHANGE] No se pudo detectar modo posición: {e} — asumiendo ONEWAY")
        return "ONEWAY"


# Detectar modo al importar
_POSITION_MODE = _get_position_mode()
...
def calcular_cantidad(par: str, balance: float, precio: float) -> float:
    ...
    if cantidad * precio < 4.0:
        cantidad_min = 4.2 / precio
...
def abrir_long(par: str, cantidad: float, precio_entrada: float,
    ...
    resp = _post("/openApi/swap/v2/trade/order",
                 _orden_params_long_open(qty_str, par))
    ...
    _post("/openApi/swap/v2/trade/order",
          _orden_sl_long(par, _format_price(sl), qty_str))
    _post("/openApi/swap/v2/trade/order",
          _orden_tp_long(par, _format_price(tp), qty_str))
...
def abrir_short(par: str, cantidad: float, precio_entrada: float,
    ...
    resp = _post("/openApi/swap/v2/trade/order",
                 _orden_params_short_open(qty_str, par))
