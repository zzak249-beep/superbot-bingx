...
def calcular_cantidad(par: str, balance: float, precio: float) -> float:
    ...
    # Inversión fija de 8 USDT por trade
    INVERSION_FIJA = 8.0

    if balance < INVERSION_FIJA:
        print(f"[EXCHANGE] Balance insuficiente para invertir ${INVERSION_FIJA} (balance=${balance:.2f})")
        return 0.0
    ...
    # Verificar que el notional final cumple mínimo BingX ($5)
    if cantidad * precio < 5.0:
        return 0.0

    print(f"[EXCHANGE] Inversión: ${cantidad * precio:.2f} USDT (fija ${INVERSION_FIJA})")
    return cantidad
