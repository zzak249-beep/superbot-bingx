"""
BingX Signal Bot v8 — PROFESSIONAL EDITION
==========================================
FIXES vs v7:
  ✅ Scanner encuentra símbolos (MIN_CONF y scores recalibrados)
  ✅ BTC_BLOCK subido a 2% (no bloqueaba todo el tiempo)
  ✅ Volume field fix en tickers BingX
  ✅ hours_held .total_seconds() (bug .seconds)
  ✅ SignalEngine singleton (no reinstanciar en cada cierre)
  ✅ SL fallback cuando ATR=0
  ✅ OB parse robusto (listas y dicts)
  ✅ WS timeout correcto
  ✅ Dashboard actualizado a v8
"""
...
_STATE: dict = {"html": (
    "<html><body style='background:#070b12;color:#00ff88;"
    "font-family:monospace;padding:40px'>"
    "<h2>⚡ BingX Bot v8 — Professional Edition</h2>"
...
async def _send_signal_alert(client, sig: dict):
    ...
    lev_info = f"ATR: {sig['atr']:.6g} | " if sig.get("atr", 0) > 0 else ""

    msg = (
        f"{emoji} <b>{lvl} 🟢 {sig['symbol']} — {score:.0f}pts</b>\n"
        f"💲 ${sig['price']:.6g}\n"
...
def _render(s: dict) -> str:
    ...
    DRY  = _DRY_RUN
    mode_badge = (
        '<span class="badge badge-dry">DRY RUN</span>' if DRY
    ...
    stats = "".join([
        ...
        stat_card("Posiciones",    len(s.get("trades",  [])),  "#34d399", "max 3"),
    ...
    for sg in sorted(s.get("signals", []), key=lambda x: x.get("score", 0), reverse=True):
        ...
        srows += (
            ...
            f'{"#34d399" if sg.get("cvd_pct", 50) >= 58 else "#f87171" if sg.get("cvd_pct", 50) <= 42 else "#fbbf24"}">'
    ...
    for t in s.get("trades", []):
        ...
        trows += (
            ...
            f'<td class="num warn">${t.get("tp1", 0):.6g} {tp1h}</td>'
            f'<td class="num good">${t.get("tp2", 0):.6g}</td>'
...
<title>BingX Bot v8 — Professional</title>
<style>
  :root {{
  ...
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  .pulse {{
    display:inline-block; width:6px; height:6px; background:#34d399;
...
<div class="hdr">
  <div class="hdr-brand">
    <div class="hdr-title">⚡ BINGX SIGNAL BOT v8</div>
    <div class="hdr-sub">PROFESSIONAL · CVD · BB SQUEEZE · MTF · ORDER BOOK · PARALELO</div>
  </div>
  <div class="hdr-right">
    <div class="hdr-meta">
...
class TradingBot:
    ...
    async def run(self):
        ...
        log.info(f"Bot v8 PROFESSIONAL iniciado — {mode} (health :{_PORT})")
        ...
        try:
            ...
            try:
                await self.client.notify(f"❌ <b>Error inicio Bot v8</b>\n{e}")
            except Exception:
                pass
            return

        await self.client.notify(
            f"⚡ <b>Bot v8 Professional iniciado</b> — {mode}\n"
        ...
        for sig in (signal.SIGTERM, signal.SIGINT):
            try:
                loop.add_signal_handler(
                    sig, lambda: setattr(self, "running", False)
                )
        ...
        while self.running:
            try:
                ...
                log.error(f"Ciclo #{self._cycle} error: {e}", exc_info=True)
    ...
    async def _tick(self):
        ...
        hot = await self.scanner.get_hot_symbols(
            top_n=TOP_N, min_volume=MIN_VOL
        )
        ...
        eval_results = await asyncio.gather(
            *[_eval_one(s) for s in hot], return_exceptions=True
        )
