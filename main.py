"""
╔══════════════════════════════════════════════════════════════════╗
║         SATY ELITE v11 — REAL MONEY EDITION [FIXED]             ║
║         BingX Perpetual Futures · 12 Trades · 24/7             ║
╠══════════════════════════════════════════════════════════════════╣
║  FIXES aplicados:                                               ║
║  · FIXED_USDT default = 10 USDT por trade (antes 8)            ║
║  · risk_mult() siempre 1.0 — nunca reduce a 0.5x               ║
║    (antes bajaba a $4 tras 3 pérdidas seguidas)                 ║
║  · set_leverage(10) automático en cada apertura                 ║
╚══════════════════════════════════════════════════════════════════╝
...
class BotState:
    ...
    def risk_mult(self) -> float:
        # Siempre 1.0 — FIXED_USDT se respeta siempre, nunca se reduce a la mitad
        return 1.0
...
def open_trade(ex: ccxt.Exchange, symbol: str, base: str,
    ...
    try:
        ...
        # ── Fijar apalancamiento 10x ──
        try:
            ex.set_leverage(10, symbol)
            log.info(f"[{symbol}] Leverage 10x ✓")
        except Exception as e:
