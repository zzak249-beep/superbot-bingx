...
FIXES v3.6 — 3 bugs que causaban apertura masiva de posiciones:
  ...
  FIX-E  RECOVERY AL REINICIAR (causa de las 13 posiciones)
         ...
         tenía "if not self.open_trades: return" → nunca sincronizaba.
         El bot veía 0 trades locales y abría nuevos sin límite,
         aunque BingX ya tuviera 13 posiciones abiertas.
         Ahora: _recover_open_positions() en __init__ reconstruye
         open_trades desde la API de BingX (shorts: positionAmt < 0).

  FIX-D  ELIMINAR reduceOnly EN HEDGE MODE (causa de cierres fallidos)
         BingX Hedge mode rechaza reduceOnly=true con error [109400].
         Eliminado de: _close_short() IOC, MARKET fallback,
         MARKET quoteOrderQty, y cierre de emergencia.
         En Hedge mode, positionSide=SHORT identifica la posición.

  FIX-F  _sync_bingx() SIN GUARD DE OPEN_TRADES
         El guard "if not self.open_trades" impedía detectar cierres
         cuando el dict local estaba vacío tras reinicio.
         Ahora corre siempre (solo verifica AUTO_TRADING).

  FIX-G  _contar_posiciones_reales() PARA SHORTS
         Nuevo método que cuenta SHORTs reales en BingX
         (positionAmt < 0) antes de cada apertura.
         Evita abrir más de MAX_TRADES aunque open_trades sea vacío.

  FIX-H  _hora_ok() CORREGIDO
         Retornaba datetime.utcnow().hour (entero 0-23, siempre truthy)
         en vez de bool. Ahora: retorna bool correcto.

  HERENCIA v3.4:
  - leverage via API antes de cada trade
  - notional = margen * LEVERAGE
  - Filtro BTC multiframe 1h+4h
  - Filtro correlación
  - MIN_SCORE=82 | RR 2.5:1
  - SL límite maker + 6 reintentos
  - Cooldown 30 min
════════════════════════════════════════════════════════════════
"""
...
class ShortBot:
    ...
    # ---------------------------------------------------------------- FIX-E: recovery al reiniciar

    def _recover_open_positions(self):
        """
        FIX-E CRITICO: Recupera posiciones SHORT abiertas desde BingX al arrancar.
        Sin esto, al reiniciar Railway open_trades={} y el bot abria nuevos trades
        sin limitar — fue la causa de las 13 posiciones en pantalla.
        SHORT: positionAmt < 0 o positionSide == 'SHORT'.
        """
        if not AUTO_TRADING:
            return
        log.info("  [RECOVERY] Buscando posiciones SHORT abiertas en BingX...")
        try:
            ...
            for p in (d.get('data') or []):
                ...
                sym  = p.get('symbol', '')
                if not sym or not (amt < 0 or side == 'SHORT'):
                    continue
                ...
                if qty <= 0 or entry <= 0:
                    continue
                # Reconstruir TP/SL conservadores desde entry
                ...
                recovered += 1
                log.info(f"  [RECOVERY] {sym}: {qty:.4f} cts @ ${entry:.6f} SHORT → TP:${tp_price:.6f} SL:${sl_price:.6f}")
            if recovered > 0:
                log.info(f"  [RECOVERY] {recovered} posiciones SHORT recuperadas")
                self._tg(
                    f"<b>♻️ RECOVERY SHORTS: {recovered} posiciones</b>\n"
                    + "\n".join(f"  • {sym}: entry ${t['entry']:.6f}" for sym, t in self.open_trades.items())
                )
            else:
                log.info("  [RECOVERY] No hay posiciones SHORT abiertas en BingX")
        except Exception as e:
            log.warning(f"  [RECOVERY] Error: {e}")

    # ---------------------------------------------------------------- FIX-G: conteo real de SHORTs
    ...
    def _contar_posiciones_reales(self):
        ...
        try:
            ...
            log.warning(f"  [REAL] Error contando: {e}")
    ...
    def _qty_contratos(self, symbol, price, margin_usdt=None):
        """
        FIX-B CRITICO: margin_usdt es el MARGEN real deseado (capital comprometido).
        El notional que se envia al exchange = margin_usdt * LEVERAGE.

        ANTES (bug): POSITION_SIZE=8 -> 8 USDT notional a 18X = 0.44 USDT margen real
        AHORA (fix): POSITION_SIZE=8 -> 8*3=24 USDT notional a 3X = 8 USDT margen real
        """
        if margin_usdt is None:
            margin_usdt = POSITION_SIZE
        # Garantia: nunca menos de FORCE_MIN_USDT de margen
        margin_usdt = max(margin_usdt, FORCE_MIN_USDT)
        # Notional = capital real * leverage
        ...
        while val_margin < min_margin and i < 500:
            qty += step; qty = round(qty, prec)
            val_notional = qty * ppc
            val_margin   = val_notional / LEVERAGE
            i += 1
        if val_margin > margin_usdt * 1.3:
            cap_notional = margin_usdt * 1.3 * LEVERAGE
            qty = round(math.floor((cap_notional / ppc) / step) * step, prec)
            val_notional = qty * ppc
            val_margin   = val_notional / LEVERAGE
            if val_margin < min_margin:
                ...
                val_notional = qty * ppc
                val_margin   = val_notional / LEVERAGE
        log.info(f"    sizing: {qty} cts x ${ppc:.6f} = ${val_notional:.2f} notional | MARGEN REAL: ${val_margin:.2f} @ {LEVERAGE}x")
    ...
    def _hora_ok(self):
        # FIX-H: retorna bool, no int (entero truthy rompia el filtro)
    ...
    def _place_short_entry(self, symbol, margin_usdt, price):
        ...
        log.info(f"  SHORT {symbol}: margen=${margin_usdt:.2f} x{LEVERAGE} = notional~${usdt_notional:.2f} | contratos={qty_c}")
        ...
        if USE_LIMIT_ORDERS and qty_c:
            ...
            if d.get('code') == 0:
                log.info(f"  ENTRADA LIMITE maker OK {qty_c} cts @ ${limit_price:.6f} (margen ~${margin_usdt:.2f})")
            ...
            log.warning(f"  Limite fallo [{d.get('code')}] — fallback mercado")

        # Fallback 1: quoteOrderQty usa el notional correcto
        d = bingx_request('POST', '/openApi/swap/v2/trade/order', {
            'symbol':symbol,'side':'SELL','positionSide':'SHORT',
            'type':'MARKET','quoteOrderQty':str(round(usdt_notional, 2)),
        }).json()
        if d.get('code') == 0:
            return d.get('data',{}).get('orderId','OK'), qty_c

        log.warning(f"  quoteOrderQty fallo [{d.get('code')}] — fallback contratos")
        ...
        log.error(f"  Todos los metodos fallaron [{d2.get('code')}]: {d2.get('msg')}")
        return None, None
    ...
    def _close_short(self, symbol, t):
        ...
        if qty_c and qty_c > 0:
            ...
            if cur_price > 0:
                ...
                d = bingx_request('POST', '/openApi/swap/v2/trade/order', {
                    ...
                    'price':str(limit_price),'timeInForce':'IOC',  # Hedge: no reduceOnly, positionSide=SHORT identifica
                }).json()
                if d.get('code') == 0:
        ...
        if qty_c and qty_c > 0:
            ...
            params = {'symbol':symbol,'side':'BUY','positionSide':'SHORT',
                      'type':'MARKET','quoteOrderQty':str(round(usdt,2))}  # Hedge: no reduceOnly
    ...
    def open_trade(self, symbol, sig):
        if not AUTO_TRADING:
            log.info(f"  [SENAL] SHORT {symbol} score:{sig['score']:.0f} regimen:{sig['regime']}")
            return False
        ...
        if not self._correlacion_ok():
            log.info(f"  {symbol} bloqueado por filtro de correlacion")
            return False
    ...
    def _open_trade_inner(self, symbol, sig):
        ...
        log.info(f"  Score:{sig['score']:.0f}/{sig['score_min']:.0f} | RSI:{sig['rsi']:.0f} | "
                 f"BB:{sig['bb_pos']}% | Regimen:{sig['regime']} | BTC 1h:{self._btc_change_1h:+.2f}% 4h:{self._btc_change_4h:+.2f}%")
        ...
        self._tg(
            ...
            f"RR: {rr}:1 | Margen: ${usdt_qty} USDT x{LEVERAGE} = ~${usdt_qty*LEVERAGE:.0f} notional\n"
    ...
    async def _sync_bingx(self):
        # FIX-F: no guard de open_trades — debe correr siempre para detectar cierres
        if not AUTO_TRADING: return
        try:
            d = bingx_request('GET', '/openApi/swap/v2/user/positions', {}).json()
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                # FIX-G: usar conteo real de BingX para decidir si analizar
                pos_reales     = self._contar_posiciones_reales()
                ...
                if slots_libres > 0:
                    if not self._correlacion_ok():
                        log.info(f"  Filtro correlacion activo — esperando cierre de posiciones en perdida")
                    elif not self._btc_regime_ok():
                        log.info(f"  Filtro BTC multiframe activo — no se analizan senales")
                    else:
                        found = 0
                    ...
                    log.info(f"  Max ({MAX_TRADES}) posiciones — esperando cierre (BingX={pos_reales} local={trades_locales})")
