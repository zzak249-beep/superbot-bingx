"""
main_smc.py — SMC Sniper Bot v1.0
Loop principal 24/7 para BingX Perpetual Futures.
Estrategia: Liquidity Sweep + QML + Stop Clusters + Supertrend
"""
...
log.info("=== ARRANQUE SMC SNIPER BOT v1.0 ===")
...
def main():
    ...
    log.info(f"  TF:{cfg.TIMEFRAME} | TP={cfg.TP_DIST_MULT}x | SL={cfg.SL_ATR_MULT}x | MIN_RR:{cfg.MIN_RR}")
    ...
    _notif(
        ...
        f"📊 Pares   : `{len(pares)}`\n"
        f"🎯 Estrategia: *SMC Sniper*\n"
        f"💧 Sweep + QML + Stop Clusters + ST\n"
        ...
        try:
            ...
            if time.time() - last_scan_pares > 3600:
                try:
                    ...
                    resto_n = [p for p in nuevos if p not in set(top_m) and p not in set(memoria.get_pares_bloqueados())]
            ...
            if _pos_bot_count() < cfg.MAX_POSICIONES:
                ...
                if senales:
                    ...
                    for s in senales[:5]:
                        log.info(f"  {s['lado']:5s} {s['par']:15s} score={s['score']} "
                ...
                for s in senales:
                    ...
                    if resultado and resultado.startswith("error:"):
                        ...
                        continue
                    if resultado and any(x in resultado for x in ("MAX_POSICIONES","exposición","margen","bloq")):
